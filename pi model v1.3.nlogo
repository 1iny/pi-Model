...
[ 
  ...
  Bo     ;; optimal biomass (without interaction)
...
to setup  
  ...
  [ ask n-of number_of_cohort patches
     [
       sprout density / number_of_cohort
         [
           plant-setting
          
           right random 360
           forward random-normal 0 (size-factor * cluster_scale / 4)            
         ]
     ]
  ]
  [  
...
;; mortality
  ...
  if number-dead > 0 and number-alive > 2
...
;; Relative Interaction Index
  ...
  [   
      ...
      plotxy (ticks + 1) (mean [B] of turtles - mean [Bo] of turtles with [Bo != 0]) / (mean [B] of turtles + mean [Bo] of turtles with [Bo != 0])
  ]
...
;; Coverage
  ...
  [  
    ...
    plotxy (ticks + 1)  ( count patches with [nb-compete != 0] * 100 / count patches)   
  ]
...
to growth
  ...
  [ set plant-dead true ]
  [ 
    let Bo-gr a * (Bo ^ (3 / 4)) * ( 1 - ((Bo / Bmax) ^ exponent) / f-r ) * f-r   ;; optimal growth, without competition      
    set Bo Bo + Bo-gr 
 
    ;; actual growth rate
    let gr a * Ae * ( 1 - ((B / Bmax) ^ exponent) / (f-r * f-c) ) * f-r
 
    ;; growth and death
    ifelse gr <= 0 or gr <= ( (B ^ ( 3 / 4 )) * threshold-of-death )
    [ set plant-dead true ]
    [ set B B + gr 
      set rad ( B ^ ( 3 / 8 ) ) * ( PI ^ ( -1 / 2 ) ) ] 
  ] 
...
density
density
1 49 100 144 256 400 676 1089 1764 3025 4900 8100 10000 12100
11
...
Copyright 2009-2013 Yue Lin. yue.lin.tud@gmail.com
...
- Lin Y, Berger U, Grimm V, Ji QR. (2012) Differences between symmetric and asymmetric facilitation matter: exploring the interplay between modes of positive and negative plant interactions. Jornal of Ecology, 100:1482-1491.

- Lin Y, Berger U, Grimm V, Huth F, Weiner J. (2013) Plant interactions alter the predictions of metabolic scaling theory. PLoS ONE, xx:xxx-xxx
...
@#$#@#$#@
<experiments>
  <experiment name="experiment woody 0.2" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>count turtles &lt;= 100</exitCondition>
    <metric>count turtles</metric>
    <metric>number-dead</metric>
    <metric>number-alive</metric>
    <metric>mean [B] of turtles with [plant-dead = false]</metric>
    <metric>mean [Bo] of turtles with [plant-dead = false]</metric>
    <metric>( 100 * standard-deviation [B] of turtles with [plant-dead = false] / mean [B] of turtles with [plant-dead = false] )</metric>
    ...
    <enumeratedValueSet variable="plant_type">
      <value value="&quot;Woody&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="threshold-of-death">
      <value value="0.02"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="resource-limitation">
      <value value="0"/>
      <value value="0.1"/>
      <value value="0.4"/>
      <value value="0.5"/>
  ...
  <experiment name="experiment non-woody 0.2" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>count turtles &lt;= 100</exitCondition>
    <metric>count turtles</metric>
  ...
  <experiment name="experiment for AN woody 0.2" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>count turtles &lt;= 100</exitCondition>
    <metric>count turtles</metric>
  ...
  <experiment name="experiment for AN non-woody 0.2" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>count turtles &lt;= 100</exitCondition>
    <metric>count turtles</metric>
